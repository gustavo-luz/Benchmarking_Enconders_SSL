...
class Sam(L.LightningModule):
    ...
    def __init__(
        ...
        self.register_buffer(
            "pixel_mean",
            torch.Tensor(pixel_mean or [123.675, 116.28, 103.53]).view(-1, 1, 1),
            False,
        )
        self.register_buffer(
            "pixel_std",
    ...
    def _apply_adapter(self, apply_adapter, alpha=1, rank=4):
        ...
        if "image_encoder" in apply_adapter:
            ...
            for layer in self.model.image_encoder.blocks:
                layer.attn.qkv = apply_adapter["image_encoder"](
                    original_module=layer.attn.qkv, bias=True, alpha=alpha, r=rank
                )
        if "mask_decoder" in apply_adapter:
            ...
            for layer in self.model.mask_decoder.transformer.layers:
                ...
                layer.cross_attn_token_to_image.q_proj = apply_adapter["mask_decoder"](
                    original_module=layer.cross_attn_token_to_image.q_proj,
                    bias=True,
                    alpha=alpha,
                    r=rank,
                )
                layer.cross_attn_token_to_image.v_proj = apply_adapter["mask_decoder"](
    ...
    def load_from_h(
        self, sam, state_dict, image_size, vit_patch_size, encoder_global_attn_indexes
    ):
        """
        ...
        if isinstance(batch, list) and len(batch) == 2:
            ...
            for sample_idx in range(len(batch)):
                sample_data = {
                    ...
                    "original_size": (
                        batch[sample_idx]["image"].shape[1],
                        batch[sample_idx]["image"].shape[2],
                    ),
                }
