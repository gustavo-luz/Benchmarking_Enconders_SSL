...
class TriBYOL(L.LightningModule):
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor, x3: torch.Tensor):
        ...
        # Target network 1
        with torch.no_grad():
            t2 = self.target_projector_1(
                torch.flatten(self.avgpool(self.target_network_1(x2)), 1)
            )
            t3 = self.target_projector_2(
                torch.flatten(self.avgpool(self.target_network_2(x3)), 1)
            )
    ...
    def update_target_networks(self):
        ...
        for online_params, target_params_1, target_params_2 in zip(
            ...
            target_params_1.data = (
                self.tau * target_params_1.data + (1 - self.tau) * online_params.data
            )
            target_params_2.data = (
                self.tau * target_params_2.data + (1 - self.tau) * online_params.data
            )
